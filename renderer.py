...
# track current tick and absolute time in seconds
tick_time = 0
absolute_time = 0

# store events with real-time position
events = [(-1, [])]

for msg in mido.merge_tracks(file.tracks):
    # Advance by this message's delta-time (in ticks and seconds)
    tick_time += msg.time
    delta_seconds = mido.tick2second(msg.time, ticks_per_beat, current_tempo)
    absolute_time += delta_seconds

    # If tempo changes, update tempo AFTER processing its time position
    if msg.type == "set_tempo":
        current_tempo = msg.tempo

    if events[-1][0] == absolute_time:
        events[-1][1].append(msg)
    else:
        events.append((absolute_time, [msg]))

# simulate stepping through time slices
current_time = 0
events.pop(0) # remove first useless element (-1, [])
current_event_batch = events.pop(0)

process = subprocess.Popen(ffmpeg_cmd, stdin=subprocess.PIPE)

frame_count = 0
# sequentially go through every frame and process the image
while len(events) > 0:
    # get all messages in this time window
    messages_this_frame = None
    # if we got to the next event batch and the difference is neglegible
    if current_event_batch[0] - current_time <= dt:
        messages_this_frame = current_event_batch[1]
        current_event_batch = events.pop(0)

    # process messages
    if messages_this_frame != None:
        for msg in messages_this_frame:
            process_message(msg)
